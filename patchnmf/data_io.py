...
def get_tiff(ds):
    tiff_all = []
    for (i, ti) in enumerate(os.listdir(f'data_proc/pixels/{ds}/downsampled_tiff')):
        print(ti)
        
        if i == 0:
            tiff = io.imread(f'data_proc/pixels/{ds}/downsampled_tiff/{ti}',  plugin='pil') # initialise tiff
            tiff = img_as_uint(tiff) 
        else:
            tiff_i = io.imread(f'data_proc/pixels/{ds}/downsampled_tiff/{ti}',  plugin='pil')
...
def get_numpy(ds):
    ...
    npy_path = os.path.join(f'data_proc/pixels/{ds}', 'preprocessed_tiff.npy')
...
def get_numpy_exploratory(ds):
    ...
    npy_path = os.path.join(f'data_proc/{ds}', 'preprocessed_tiff_no_thresholding.npy')
    data = np.load(npy_path)
    print(f' Data shape : {data.shape}')
    return data 
...
def read_params(pixelnmf_params, ds):
   ...
   # return params_table, blur_std, downs_fact, res_orig, x_axis, y_axis, original_FOV, framerate, duration, resolution, time_per_frame, n_components_elbow, n_components_min, age, weight_g, s2p, cells, Behaviour, SLEAP 
