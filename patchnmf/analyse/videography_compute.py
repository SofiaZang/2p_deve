...
#from jure's code for all ds 
#plot zero lagged correlations with motion energy
def get_all_corr_mot_en(all_f_t2p, all_mot_energy):
    all_corr_mot_en = []
    for (i, f) in enumerate(all_f_t2p):
        # loop through neurons
        mot_en = all_mot_energy[i]
        corr_mot_en = []
        for j in range(f.shape[0]):
            corr_mot_en.append(np.corrcoef(f[j], mot_en)[0, 1])
            if np.isnan(corr_mot_en[-1]):
                corr_mot_en[-1] = 0
        corr_mot_en = np.array(corr_mot_en)
        all_corr_mot_en.append(corr_mot_en)

    return all_corr_mot_en

def compute_all_pmth(all_act, all_beh_bin, pmth_wind=[10,30]):
    
    all_pmth = []

    for (act, beh) in zip(all_act, all_beh_bin):


        # get all onsets of movement
        onsets = np.where(np.diff(beh.astype(int)) == 1)[0]

        # get the activities around the onsets
        act_onsets = np.zeros((len(onsets), act.shape[0], sum(pmth_wind)))

        for i, onset in enumerate(onsets):
            if onset < pmth_wind[0] or onset > len(beh) - pmth_wind[1]: # dealing with edge cases
                continue
            act_onsets[i] = act[:, onset-pmth_wind[0]:onset+pmth_wind[1]]

        pmth = np.mean(act_onsets, axis=0)
        all_pmth.append(pmth)
    return all_pmth